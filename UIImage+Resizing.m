...
{
    UIGraphicsBeginImageContextWithOptions(self.size, YES, 1.0f);
    
    [self drawInRect:CGRectMake(0, 0, self.size.width, self.size.height)];
    //center
    int x = (self.size.width-image.size.width)/2;
    [image drawInRect:CGRectMake(x, self.size.height - image.size.height, image.size.width, image.size.height)];
...
-(UIImage *) drawImage: (UIImage *)image withsize:(CGSize)newSize
{
	size_t destWidth = (size_t)(newSize.width * self.scale);
	size_t destHeight = (size_t)(newSize.height * self.scale);
	if (self.imageOrientation == UIImageOrientationLeft
    ...
    /// Create an ARGB bitmap context
	CGContextRef bmContext = NYXCreateARGBBitmapContext(self.size.width,self.size.height, self.size.width * kNyxNumberOfComponentsPerARBGPixel, NYXImageHasAlpha(self.CGImage));
	if (!bmContext)
		return nil;
	...
	UIGraphicsPushContext(bmContext);
    //center
     float x = (self.size.width-image.size.width)/2;
    CGContextDrawImage(bmContext, CGRectMake(x, 0.0f, image.size.width, image.size.height), image.CGImage);
	UIGraphicsPopContext();
    
	/// Create an image object from the context
	CGImageRef scaledImageRef = CGBitmapContextCreateImage(bmContext);
	UIImage* scaled = [UIImage imageWithCGImage:scaledImageRef scale:self.scale orientation:image.imageOrientation];
...
+(UIImage*) CreateFillImage:(CGSize) imageSize withColor:(UIColor* )fillColor
 {
